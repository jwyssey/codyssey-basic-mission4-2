...
실행 바이너리는 해당 환경의 아키텍처와 호환되어야 한다. 과제 1의 정상 앱을 과제 2 앱으로 이름만 바꾸어 사용하면 안 된다. 원본 ZIP은 저장소의 `vendor/agent-app-leak.zip`에 포함되어 있다. `unzip -l vendor/agent-app-leak.zip`으로 목록을 보고 [vendor 안내](../vendor/README.md)에 따라 실행 파일만 추출한다. 패키징된 실행 파일 내부를 풀거나 디컴파일하지 않는다.
