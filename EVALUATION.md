...
아래는 자동 수집한 자료를 **대상 식별 → 생존 확인 → 진행 여부 확인 → 대기 원인 분석 → 변경 후 검증** 순서로 해석한 과정이다. 명령을 수동으로 이 순서에 맞춰 실행했다는 뜻은 아니다. 1~4단계는 같은 `deadlock-before` 실행의 기록이고, 5단계는 설정을 바꾼 별도의 `deadlock-after` 실행이다.

#### ① 먼저 실제 작업을 맡은 PID를 찾는다

제공 앱은 실행기와 워커가 서로 다른 PID로 나타난다. 따라서 실행기를 관측하고 정상 여부를 판단하지 않도록, 앱이 사용하는 15034 포트의 소유자를 먼저 확인했다. 다음은 `snapshots.txt`의 명령과 출력 발췌다.

```text
[2026-09-18T09:14:44.530+00:00] $ ss -ltnp 'sport = :15034'
State  Recv-Q Send-Q Local Address:Port  Peer Address:PortProcess
LISTEN 0      1            0.0.0.0:15034      0.0.0.0:*    users:(("agent-leak-app-",pid=2448740,fd=4))
```

`pid=2448740`이 15034 포트를 소유한 프로세스다. 같은 스냅샷의 `ps` 명령과 필요한 열을 함께 확인했다. 표에서는 긴 실행 경로 등 다른 열을 생략했다.

```bash
ps -p 2448732,2448740 -o user,pid,ppid,pgid,etime,stat,pcpu,pmem,rss,vsz,args
```

| PID | PPID(부모 PID) | PGID(프로세스 그룹) | STAT | 해석 |
| ---: | ---: | ---: | --- | --- |
| 2448732 | 2448712 | 2448732 | Ss | 앱 실행기 |
| 2448740 | 2448732 | 2448732 | SNl | 실행기의 자식이며 포트를 소유한 워커 |

**따라서 이후 CPU·메모리·스레드 분석의 대상은 워커 PID 2448740으로 정했다.** 포트가 `LISTEN` 상태라는 사실만으로 요청 처리가 정상이라고 판단하지는 않았다. 프로세스가 멈춰 있어도 소켓은 열린 채 남을 수 있기 때문이다.

#### ② 같은 프로세스가 계속 살아 있는지 확인한다

한 번 PID가 보인 것만으로는 부족하다. 프로세스가 종료된 뒤 같은 PID가 재사용될 수도 있으므로, `metrics.csv`의 PID와 `start_ticks`(커널이 기록한 프로세스 시작 시점)를 함께 비교했다. 이 값들은 수집기가 `/proc/PID/stat`에서 읽은 것이다.

다음은 실제 CSV의 첫 행과 마지막 행에서 필요한 열만 추린 것이다.

```csv
timestamp,pid,start_ticks,state
2026-09-18T09:14:34.158+00:00,2448740,21797426,S
2026-09-18T09:16:03.649+00:00,2448740,21797426,S
```

중간 샘플에서도 PID와 시작 틱은 같았다. 마지막 스냅샷에 보존된 `/proc/2448740/status`의 상태 역시 `State: S (sleeping)`이었다. `S`는 무언가를 기다리며 잠든 상태이며, 종료 후 정보만 남은 좀비 상태 `Z`와 다르다.

**이 단계에서 확인한 것은 “같은 프로세스가 관찰 종료 직전까지 존재했다”는 사실이다.** 아직 정상 동작 여부는 알 수 없으므로 다음 단계에서 실제 작업이 진행되는지 확인했다.

#### ③ CPU·메모리와 로그가 함께 정체됐는지 확인한다

`metrics.csv`에서 해당 워커의 시간대별 측정값을 비교했다. 아래는 필요한 열만 추린 실제 샘플이다.

```csv
timestamp,pid,rss_kib,cpu_percent
2026-09-18T09:14:44.718+00:00,2448740,17664,0.00
2026-09-18T09:15:20.409+00:00,2448740,17664,0.00
2026-09-18T09:16:03.649+00:00,2448740,17664,0.00
```

이 구간의 나머지 샘플도 CPU는 0.00%, RSS는 17664KiB(17.250MiB)로 유지됐다. 이어서 `log-sizes.jsonl`에 저장한 로그 파일 크기를 비교했다.

| 측정 시각(UTC) | `console.log` | `app-logs/agent_app.log` |
| --- | ---: | ---: |
| 09:14:44.539 | 2881 bytes | 1517 bytes |
| 09:15:21.987 | 2881 bytes | 1517 bytes |
| 09:16:03.856 | 2881 bytes | 1517 bytes |

처음과 마지막 시각의 차이는 **79.317초**이며, 그 사이의 기록도 같은 크기였다. 앱 로그는 뒤에 추가하는 방식으로 수집했고, 내용의 마지막에는 두 스레드의 `WAITING`이 남아 있었다.

**같은 프로세스가 살아 있으면서 측정된 CPU·RSS와 작업 로그가 장시간 정체됐으므로, 작업 진행이 멈춘 상태를 의심했다.** 다만 정상적인 입력 대기도 비슷하게 보일 수 있다. 이 단계만으로 Deadlock을 확정하지 않고, 무엇을 기다리는지 더 확인했다.

#### ④ 스레드의 대기 상태를 확인하고 락 로그로 원인을 좁힌다

관찰 종료 직전 `ps -L` 출력이다. `-L`은 프로세스 내부의 스레드를 각각 보여 준다.

```text
[2026-09-18T09:16:03.609+00:00] $ ps -L -p 2448732,2448740 -o pid,tid,stat,pcpu,rss,wchan:32,comm
    PID     TID STAT %CPU   RSS WCHAN                            COMMAND
2448732 2448732 Ss    0.0  2048 do_wait                          agent-leak-app-
...
실행기 2448732는 `do_wait`에서 자식 프로세스를 기다렸고, 워커 2448740의 세 스레드는 `futex_wait_queue`에서 동기화 대기 중이었다. `PID`는 소속 프로세스, `TID`는 각 스레드의 번호다. `STAT`의 `S`는 대기, `N`은 낮춘 우선순위, `l`은 멀티스레드 프로세스임을 뜻한다.

같은 시점의 `top -b -H -n 1 -p 2448732,2448740`에서도 다음과 같이 관측됐다.

```text
Threads:   4 total,   0 running,   4 sleeping,   0 stopped,   0 zombie
```

네 스레드는 실행기의 한 스레드와 워커의 세 스레드를 합한 것이다. `futex` 대기는 정상 동기화에서도 나타나므로 이것만으로 교착의 원인을 알 수는 없다. **어떤 자원을 소유하고 무엇을 기다리는지**는 앱의 `console.log`로 확인했다.
...
앱 로그는 KST이므로 마지막 대기 시각 `18:14:42.880`은 UTC `09:14:42.880`이다. 이후 ③에서 로그 정체가 관측된 시간과 연결된다.

| 스레드 | 이미 소유한 락 | 추가로 기다리는 락 | 기다리는 락의 소유자 |
| --- | --- | --- | --- |
| Thread-1 | A | B | Thread-2 |
| Thread-2 | B | A | Thread-1 |

Thread-1이 진행하려면 Thread-2가 B를 놓아야 하지만, Thread-2도 Thread-1이 A를 놓기를 기다린다. 이후 락 해제·작업 완료 로그가 없고 OS 관측도 정체돼 있으므로, **서로의 자원을 기다리는 순환 대기로 작업이 멈췄다**고 판단했다.

여기서 `Worker-Thread-1/2`라는 앱의 이름을 특정 OS TID에 일대일로 대응시킨 것은 아니다. 앱 로그로 소유·대기 관계를, OS 기록으로 프로세스 생존과 스레드 대기를 확인해 함께 해석했다.

#### ⑤ 설정을 바꾼 새 실행에서 작업이 계속되는지 검증한다

마지막으로 `MEMORY_LIMIT=512`, `CPU_MAX_OCCUPY=40`, 관찰 상한 90초와 수집 간격 0.5초를 고정하고, `MULTI_THREAD_ENABLE`만 `true → false`로 바꿔 새로 실행했다. After의 `console.log`에는 다음 기록이 남았다.
...
초기 작업 완료 뒤에도 메모리 회수와 새 작업 로그가 이어졌다. 두 실행의 `result.json`은 다음처럼 비교할 수 있다.

| 항목 | Before | After |
| --- | --- | --- |
| `observed_s` | 90.021 | 90.044 |
| `reason` | observation_timeout | observation_timeout |
| `alive_before_cleanup` | true | true |
| 실행기가 보낸 정리 신호 | SIGTERM | SIGTERM |
| 앱의 작업 진행 | 락 대기 이후 로그 정체 | 작업 완료·메모리 회수·로그 갱신 |

**두 실행 모두 프로세스가 살아 있었다는 점은 같지만, After에서만 작업 진행이 확인됐다.** 따라서 PID의 존재보다 작업 완료와 로그 갱신을 함께 확인해야 정상 여부를 판단할 수 있다. 이 비교는 설정 변경 후 새 실행에서 교착을 회피했다는 근거이며, 이미 교착된 프로세스의 락을 풀거나 내부 락 설계를 수정했다는 뜻은 아니다.

자료 위치: `evidence/runs/20260918T091433Z-deadlock-before-531338111/` 및 `evidence/runs/20260918T091713Z-deadlock-after-166295630/`의 `snapshots.txt`, `metrics.csv`, `log-sizes.jsonl`, `console.log`, `result.json`. 위 자료는 링크를 열지 않고도 진단 흐름을 따라갈 수 있도록 필요한 부분을 본문에 발췌했다.
